...
/**
 * Similar to useShallow from zustand: https://zustand.docs.pmnd.rs/guides/prevent-rerenders-with-use-shallow
 * only this works on does not require a store selector function
 */
export function useShallowObjectSelectorMemo<S, T = Partial<S> | S[keyof S][]>(store: S, selector: (store: S) => T): T {
  const prev = useRef<T>();
